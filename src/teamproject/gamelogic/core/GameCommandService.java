...
public class GameCommandService
	...
	private void addEnemies(World world) {
	...
	private Game generateNewLocalGame(final String localUsername, final int localPlayerID, final GameSettings settings) {
		// Generate a map
		...
		final Game game = new Game(world, settings, player, false);
	...
	public void onSingleplayerGameStarting(SingleplayerGameStartingEventArgs args) {
		Game g = generateNewLocalGame(args.getUsername(), 0, args.getSettings());
		getGameStartedEvent().fire(g);
		addEnemies(g.getWorld());
	}
	...
	public void onMultiplayerGameStarting(MultiplayerGameStartingEventArgs args) {
		...
		if(args.isServer()) {
			...
			g = generateNewLocalGame(args.getLocalUsername(), args.getLocalPlayerID(), args.getSettings());
			getGameStartedEvent().fire(g);
		}
	}
}
