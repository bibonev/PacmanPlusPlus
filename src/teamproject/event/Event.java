 ...
 * @author Tom Galvin
 *
 * @param <TListener> The interface which will listen to this event.
 * @param <TEventArgs> The type of argument which will be passed to the
 * event listener.
 */
public class Event<TListener, TEventArgs> {
	 ...
	 * 
	 * @param trigger A consumer function which calls the event listening
	 * function on a listener, passing the event arguments to it.
	 */
	public Event(BiConsumer<TListener, TEventArgs> trigger) {
	 ...
	 * @throws IllegalArgumentException Thrown when the given listener object
	 * is already listening to this event.
	 */
	 ...
	 * 
	 * @param listener The listener to remove (ie. stop listening).
	 ...
	 * @return Returns {@code true} if {@code listener} is listening to this event;
	 * {@code false} otherwise.
	 */
	public boolean isListenedToBy(TListener listener) {
		return listeners.contains(listener);
	}
	
	/**
	 * Fires this event, triggering all of the event listeners which are listening to it.
	 * This will block until all of the event listeners have finished running, and the
	 * order in which the event listeners are triggered is unspecified.
	 * 
